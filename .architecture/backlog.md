...
- **First seen**: 2026-09-11
- **Re-check 2026-09-22**: friction not re-verified in depth this run (`properties/proposer.py` and `core/propose.py` unchanged since 2026-09-21). Heat re-measured: W=5 → 3; score unchanged at 17/25.

## store-column-registry

- **Status**: proposed
- **Score**: 13/25 (leverage 2, locality 4, blast radius 2, heat 1)
- **Files**: ~2 estimated
...
- **Re-check 2026-09-22**: `properties/store.py` unchanged. Heat re-measured: W=1 → 1, so 15/25 → 13/25.
...
- **Re-check 2026-09-22**: filter still applies (leverage 1). #306 only added exports to `properties/__init__.py`.
...
- **Re-check 2026-09-22**: friction intact (`core/propose.py:70-80`, `core/submit.py:77-90`). New nuance from #305: `main` is now renamed away under **two** spellings, `__forseti_unused_main` (`orchestrator/ports.py:32`, text rewrite) and `__forseti_source_main` (`precond/synth.py:73`, a `#define` bracket). The new `harness-main-rename-owner` would remove the root of this entry's raw-vs-renamed asymmetry. Heat re-measured: W=3 → 2, so 18/25 → 16/25.

## counterexample-fired-label-predicate

- **Status**: proposed
- **Score**: 14/25 (leverage 2, locality 4, blast radius 2, heat 2)
- **Files**: ~3–4 estimated
...
- **Re-check 2026-09-22**: `precond/discharge.py` unchanged since the 2026-09-21 rescope; the blame scan at `discharge.py:615` survives. The Score line now carries the 2026-09-21 rescope (it still read 17/25). Heat re-measured: W=3 → 2, so 15/25 → 14/25.

## esbmc-caller-openings-module-split

- **Status**: proposed
- **Score**: 14/25 (leverage 3, locality 4, blast radius 3, heat 1)
- **Files**: ~4–6 estimated
...
- **First seen**: 2026-09-02
- **Re-check 2026-09-22**: `esbmc/units.py` has had no commit in the last 60 (W=0), so heat drops to 1: 17/25 → 14/25.
...
- **Re-check 2026-09-22**: heat re-measured: W=7 → 4 (`core/check.py` and `precond/verify.py` are hot), so 15/25 → 16/25. Still blast radius 4, still deferred for a human's before/after differential.
...
- **Status**: proposed
- **Score**: 12/25 (leverage 2, locality 3, blast radius 2, heat 1)
- **Files**: ~3 estimated
...
- **Re-check 2026-09-22**: `forseti_gate.py` has had 1 commit in the last 60. Heat re-measured: W=1 → 1, so 15/25 → 12/25.

## adapter-install-skeleton-two-harnesses

- **Status**: proposed
- **Score**: 16/25 (leverage 4, locality 3, blast radius 3, heat 2)
- **Files**: ~5 estimated
...
- **Re-check 2026-09-22**: the three `install.py` files are unchanged. The Score line now carries the 2026-09-18 re-score (it still read 14/25). Heat re-measured: W=2 → 2, so 17/25 → 16/25.
...
- **Re-check 2026-09-22**: friction intact. The four wrappers are at `post_tool_use.py:43-55`, `stop_gate.py:231-250`, `codex/verify_hook.py:184-207`, `oh_my_pi/verify_hook.py:295-313`. `post_bash.py:44-73` still emits no `gate.decision` and still hand-rolls the decision and exit code at `:69`/`:72`. The target `record_property_proposed` has moved to `core/events.py:71-90`, and `core/_cli_trace.py` (#307) is now a second example of a Core helper owning an emission. Heat re-measured: W=8 → 5, so 17/25 → 18/25.
...
- **Re-check 2026-09-22**: the hooks are unchanged. Heat re-measured: W=4 → 3, so 16/25 → 15/25.
...
- **Re-check 2026-09-22**: friction intact. `_unit_slug` is at `orchestrator/persistence.py:30-41`; the 2026-09-21 ref `:174-185` was wrong, and that file has not changed. `_harness_filename` moved to `orchestrator/check.py:440-449` with #306. **Caveat**: `_unit_slug`'s only callers (`persist_run`, `persist_property_check`) have no caller in `src/` (see `orchestrator-run-record-append`), so one of the two derivations is dead in production and deletion may beat merging. Heat re-measured: W=3 → 2, so 18/25 → 17/25.
...
- **Re-check 2026-09-22**: #307 moved the trace wrapper out of `_precond_cli.py`; the two handler skeletons remain. Heat re-measured: W=2 → 2, so 15/25 → 14/25.
...
- **Re-check 2026-09-22**: filter still applies (leverage 1). #306 added `max_len` to both tools' typed parameter lists, and those lists are still the MCP schema.
...
- **Re-check 2026-09-22**: filter still applies. `forseti_gate.py` is unchanged since the last re-check.
...
- **Re-check 2026-09-22**: filter still applies (leverage 1). `_run_harness_action` still owns the generic shape.
...
- **Re-check 2026-09-22**: filter still applies. `esbmc/__init__.py` is unchanged.
...
- **Re-check 2026-09-22**: filter still applies. #306 widened `RenderedHarness` (`orchestrator/ports.py`) with `length_bounds`, so the port carries more, not less.
...
- **Re-check 2026-09-22**: filter still applies. Each epilogue still renders a different payload.
...
- **Re-check 2026-09-22**: filter still applies. The adapters are unchanged since 2026-09-21.
...
- **Status**: proposed
- **Score**: 19/25 (leverage 4, locality 5, blast radius 3, heat 3)
- **Files**: ~10 estimated
...
- **Re-check 2026-09-22**: friction intact and the code is unchanged. The six sites are `forseti_gate.py:803`/`:1646`, `property_gate.py:257`, `codex/verify_hook.py:81`, and `oh_my_pi/verify_hook.py:114` and `:137`. The clip is still `[:800]` vs `[:400]`, and build flags are still forwarded only at `property_gate.py:311-322`. New since #306: `property_gate.py:303-306` passes an explicit `--unwind 4 --unwind-ladder ""`, so the Claude Stop gate's capped `(ptr, len)` properties can never settle (a finding for a human, recorded in the 2026-09-22 report). Heat re-measured: W=5 → 3, so 20/25 → 19/25.
...
- **Re-check 2026-09-22**: friction intact and **grown**.
  - The twin blocks are now `core/cli.py:547-585` (`check`) and `:755-793` (`semantic-loop`). They are byte-identical apart from the `--json` help (`:577` vs `:785`) and the passthrough example (`:583` vs `:791`).
  - #306 added `--max-len` through a new `_add_max_len_argument` (`:512-530`). That is the third partial extraction that pulls out one flag and stops.
  - #306 then edited both handler forwards by hand (`_run_check` `:636-652`, `_semantic_loop` `:885-905`). They forward the same six parsed values; only the timeout keyword differs (`timeout_s=` vs `check_timeout_s=`).
  - The predicted drift has now happened, identically in both copies. Since #306, a `--max-len ≥ 16` adds an N+1 rung, yet both `--unwind-ladder` helps still say "whichever of 8,16 exceed --unwind".
  - **Rescoped** to take the handler-kwargs half as well (the `add_verify_arguments`/`verify_kwargs` precedent at `esbmc/verify_cli.py:74-124`). Four call sites simplify, so leverage 3 → 4.
...
- **Re-check 2026-09-22**: friction intact and grown. The same 11 `class FakeVerify` modules remain, and there are 4 more doubles under other names: `RecordingVerify` at `tests/orchestrator/test_check.py:293` and `test_loop.py:262`, `_ScriptedVerify` at `test_state_properties.py:99`, and `_RecordingVerify` at `tests/esbmc/test_esbmc_cli.py:45`. That is 15 in total. There are now 104 `FakeVerify(` call sites (#306 added ~6). The "35 `RunMeta` sites" does not reproduce: 24 files build `RunMeta(` directly and 23 have local `_meta` helpers. Heat re-measured: W=5 → 3, so 20/25 → 19/25.

## scanned-source-carrier

- **Status**: proposed
- **Score**: 16/25 (leverage 4, locality 4, blast radius 3, heat 1)
...
- **Re-check 2026-09-22**: friction intact and `esbmc/units.py` is unchanged. The scope-warning callers moved: `precond/synth.py:591` and `properties/harness.py:689` (inside `_defines_main`). Heat re-measured: **W=0** (no commit touches `units.py` in the last 60), so heat 5 → 1 and 20/25 → 16/25. The 2026-09-18 heat of 5 was an estimate.
...
- **Re-check 2026-09-22**: unchanged: codex `:120-181` ("verify" at `:153`) and omp `:217-286` ("check" at `:266`). Heat re-measured: W=5 → 3, score unchanged at 19/25.
...
- **Re-check 2026-09-22**: friction intact. **Correction** to the 2026-09-21 note: `find_open_callers` *does* run, in `tests/precond/test_discharge.py:679-713` through the default `open_callers_fn`, but all five openings are empty there, so swapping `escaped=`/`aliased=` still passes. The pairing is executed but not checked. Heat re-measured: W=3 → 2, so 18/25 → 17/25.
...
- **Re-check 2026-09-22**: not re-verified in depth. Heat re-measured: W=9 → 5 (`core/cli.py`), so 17/25 → 18/25.

## pointer-length-pairing-two-parsers

- **Status**: proposed
- **Score**: 16/25 (leverage 4, locality 4, blast radius 4, heat 2)
- **Files**: ~5 estimated
...
- **Re-check 2026-09-22**: **widened by #306**. It adds no third pairing parser (`plan_length_bounds` at `properties/harness.py:390-437` reuses `signature.py`'s `BufferParam.length`), but it makes the element-vs-byte drift user-visible. `core/cli.py:515` says `check --max-len` "Mirrors `synth --max-len` (same name, same default)", yet for `f(int *p, size_t len)`, `--max-len 8` caps 8 *ints* under `check` and 8 *bytes* under `synth`, and the difference now shows in the published `length_bounds` field. For `int sum(const int *a, int k)`, `check` caps `k` while `synth` pairs nothing, because `k` is not on its allowlist. The cap is emitted two ways: `harness.py:226-227` always emits `>= 0`, while `synth.py:333-336` omits it for unsigned types. The check path's `DEFAULT_MAX_LEN` is owned by `precond/synth.py:63`. Heat re-measured: W=2 → 2, so 17/25 → 16/25.
...
- **Re-check 2026-09-22**: not re-verified in depth. Heat re-measured: W=9 → 5; the measure is inflated by `core/cli.py`, while `update_notice.py` itself is cold. 16/25 → 19/25.
...
- **Re-check 2026-09-22**: `esbmc/units.py` and `runner.py` are unchanged (W=0). 15/25 → 12/25.
...
- **Re-check 2026-09-22**: heat re-measured: W=3 → 2, so 14/25 → 13/25.
...
- **Outcome**: reconciled `in-flight` → `landed`. PR #307 merged 2026-09-21T15:22:25Z (`gh pr view 307 --json state,mergedAt`); on `origin/main` as `2840647`.
...
- **Re-check 2026-09-22**: friction intact and the code is unchanged. The four copies are at `precond/verify.py:243-245` and `:277`, and `discharge.py:344` and `:385-388`. The CLI sites moved with #307 to `core/_precond_cli.py:61-65`, `:151-155` and `:215-219`. **Correction**: `emit_obligations` *is* tested; `tests/precond/test_discharge.py:642-670` pins its `SynthError → NEEDS_CONTRACT` and `OSError → ERROR` mappings. Only the default lister lambda is unexercised. Heat re-measured: W=6 → 4, score unchanged at 20/25. **Runner-up candidate 2026-09-22**, 2 points behind the pick.

## sibling-snapshot-staging-seam

- **Status**: proposed
- **Score**: 18/25 (leverage 4, locality 5, blast radius 2, heat 1)
- **Files**: ~2–3 estimated
...
- **Re-check 2026-09-22**: friction intact and `forseti_gate.py` is unchanged. The refs are `_enumerable_source` `:554-766` and `_verifiable_source` `:1486-1643`, and all three drifts are intact. The verify side does have 3 direct tests (`tests/adapters/claude_code/test_gate_pointer.py:1678`, `:1710`, `:1732`), but none covers exclude, the kernel directory or cleanup. Heat re-measured: W=1 → 1, so 20/25 → 18/25.
...
- **Re-check 2026-09-22**: bug still unfixed. `renderability_reason` is now at `properties/harness.py:310-387`, and `render_semantic_harness` at `:146-242` delegates to it at `:177-189`. #306 added a public `build_property_harness` (`:270-285`) on the same unvalidated path. Heat re-measured: W=6 → 4, score unchanged at 19/25.

## stop-gate-turn-outcome-carrier

- **Status**: proposed
- **Score**: 17/25 (leverage 4, locality 4, blast radius 2, heat 1)
- **Files**: ~2–3 estimated
...
- **Re-check 2026-09-22**: friction intact and the file is unchanged. **Corrected refs**: `main` `:252-461`; `outstanding` `:309` and again `:353`; `decision` `:372`; the event tails at `:373-383`, `:409-422` and `:435-448`; sections `:394-401`; `n_out` `:403`; the mislabel warning at `:368-371` (not `:298-303`). Heat re-measured: W=1 → 1, so 19/25 → 17/25.

## gate-state-carrier

- **Status**: proposed
- **Score**: 17/25 (leverage 4, locality 4, blast radius 3, heat 2)
- **Files**: ~8 estimated
...
- **Re-check 2026-09-22**: friction intact. **Count correction**: 13 `state: dict[str, Any]` annotations under `adapters/` (11 in `forseti_gate.py`, 2 in `property_gate.py`), not 23. The `stop_gate.py` refs are `:298`, `:310`, `:312` and `:349-351`. Heat re-measured: W=3 → 2, so 18/25 → 17/25.
...
- **Re-check 2026-09-22**: friction intact and **grown**. #306's now-public `plan_length_bounds` adds two more silent-skip filters (`properties/harness.py:420-421`), on top of `renderability_reason`'s two (`:363`, `:386`): 5 walks and 7 `isinstance` sites. The refs moved: the raising loop is at `:194-202`, `_nondet_ctypes` at `:488-505` (silent skip at `:501-502`), and `_call_arg` at `:553-560` (dead raise at `:560`). Heat re-measured: W=2 → 2, so 18/25 → 17/25.
...
- **Re-check 2026-09-22**: unchanged. Heat re-measured: W=1 → 1, so 18/25 → 16/25.
...
- **Re-check 2026-09-22**: friction intact, with **one more consumer**. `_semantic_loop_fields` (`core/cli.py:825-838`, #303/#307) writes the hyphen spelling into the `cli.command` event's `mode`, so the hyphen form is now also part of `.forseti/events.jsonl`. That reinforces the "additive, never a rename" constraint. The refs are `LoopMode` at `loop.py:80`, argparse choices at `cli.py:684`, the cast at `:880-882`, and MCP at `mcp_server.py:295-297`. Heat re-measured: W=9 → 5, so 18/25 → 19/25.

## assessment-vocabulary-four-tables

- **Status**: proposed
- **Score**: 16/25 (leverage 3, locality 4, blast radius 3, heat 3)
- **Files**: ~5 estimated
...
- **Re-check 2026-09-22**: not re-verified in depth. Heat re-measured: W=4 → 3, so 17/25 → 16/25.
...
- **Re-check 2026-09-22**: friction intact, plus one more gap. Core's `property.verdict` (`core/check.py:237-245`) leaves out #306's `length_bounds`, so a capped HELD reads as unbounded in `events.jsonl` while `--json` carries the `len<=N` qualifier. Fixing that is a wire change. Heat re-measured: W=7 → 4, score unchanged at 17/25.
...
- **Re-check 2026-09-22**: heat re-measured: W=2 → 2, so 15/25 → 14/25.
...
- **Re-check 2026-09-22**: the dead-surface question is wider than two functions. `orchestrator/loop.py` (`run_loop`), `fix.py`, `report.py`, `state.py`, `transcript_for` and `persist_run` have no callers in `src/`, `demo/` or `examples/`, only tests; `transcript.py` is half-used (`property_check_transcript` is imported at `core/cli.py:106`). So the question for a human covers about 600 lines, not two functions. Heat re-measured: W=1 → 1, so 13/25 → 12/25.
...
- **Re-check 2026-09-22**: filter still applies. Unchanged.
...
## check-settings-carrier

- **Status**: proposed
- **Score**: 17/25 (leverage 3, locality 4, blast radius 4, heat 5)
- **Files**: ~8–9 estimated
- **Modules**: `src/forseti/core/check.py:76-129`, `:202-217`; `core/loop.py:50-58`, `:133-140`, `:162`, `:220-233`; `core/cli.py:636-652`, `:885-905`; `core/mcp_server.py:221-232`, `:299-318`
- **Summary**: "How to check" has no name. `unwind`, `unwind_ladder`, `timeout_s`, `extra_flags`, `esbmc_bin`, `max_len` and `verify_port` are threaded as loose keywords through six layers. `run_semantic_loop` passes all of them straight through, and needs the `check_timeout_s` prefix only to tell its timeout apart from the proposer's. Validation is split by knob and ordered differently:
  - `max_len` fails before anything runs (`loop.py:162`, `check.py:203`).
  - A bad ladder fails only inside the driver (`orchestrator/check.py:309`), after `property.check.start` is written and, in `semantic-loop --mode propose`, after the LLM call.
  - `validated_max_len`'s docstring claims it mirrors `validated_ladder`; it does not.

  A frozen `CheckSettings` carrier, with `.ladder` and `.verify_port(source)` and validated at construction, would own all of it.
- **First seen**: 2026-09-22
- **Reason**: blast radius 4, because it changes the signatures of `check_source` and `run_semantic_loop`, both exported from `core/__init__.py`. Leverage 3, because the MCP faces keep their flat typed parameter list (it *is* the tool schema; see the dropped `mcp-server-tool-wrappers`), so only the Core layers and validation concentrate. It overlaps `cli-check-phase-argument-block` only at the CLI's args→kwargs translation, which that entry takes without touching an exported signature. Moving ladder validation earlier also drops a `property.check.start` line on one error path, a wire change to call out. Pins exist and need no esbmc (`tests/core/test_core_check.py:108-427`, `tests/core/test_core_loop.py:387-420`), but nothing tests MCP forwarding of `unwind`/`unwind_ladder`/`timeout_s`.

## harness-main-rename-owner

- **Status**: proposed
- **Score**: 17/25 (leverage 3, locality 4, blast radius 2, heat 3)
- **Files**: ~8 estimated
- **Modules**: `src/forseti/orchestrator/ports.py:24-32`, `:53-108` (`Unit.from_path`); `properties/harness.py:182-185`, `:670-689` (`_defines_main`); `precond/synth.py:69-73`, `:448-450`; `properties/proposer.py:77-78`
- **Summary**: The module that emits the colliding `int main(void)` (`harness.py:218`) does not own avoiding the collision. It refuses only a leftover *definition*. Its docstring (`harness.py:678-687`) calls the rest "`Unit.from_path`'s job", so the guarantee is split across two packages and linked by prose. Since #305, "rename `main` away" exists twice: a text rewrite to `__forseti_unused_main` on the check path, and a `#define main __forseti_source_main` bracket on the precond path. The rule "a unit that *is* `main` is called by its renamed name" is also written twice (`ports.py:107`, `synth.py:449`).
- **First seen**: 2026-09-22
- **Reason**: a partial deletion test. On the check path the rename concentrates into the renderer, and `_defines_main`, its `HarnessError` path and `Unit`'s second symbol are deleted. Across the two renderers only the constant and the callee rule concentrate: one path pastes the source in, the other `#include`s it. Verified by running: the text rewrite leaves `int (*g)(void) = main;` untouched, while the `#define` path handles it. That makes it a corrective follow-up for a human. The signature must be extracted *after* the rename, or a commented-out `int main(...) {` wins (see `signature-definition-locator`). `forseti check --json`'s `harness_source` and `synth --emit-only` stay byte-identical only if both spellings are kept. No esbmc-free test runs `check_source` over a file defining `main` and asserts the harness text, so that pin must land first.

## precond-sidecar-verdict-choreography

- **Status**: proposed
- **Score**: 17/25 (leverage 3, locality 4, blast radius 2, heat 3)
- **Files**: ~4–5 estimated
- **Modules**: `src/forseti/precond/verify.py:282-318` (`_run`), `:321-368` (`_assess_non_vacuity`); `precond/discharge.py:596-692` (`_check_caller`); `precond/run.py:199-217` (`SidecarRunner.probe`)
- **Summary**: Both precond drivers hand-write the same walk: climb the unwind ladder → handle `Violated` → handle anything not `Verified` → probe reachability at the settled k → a three-arm `match` ending in `assert_never`. The rule "only probe after a Verified climb" exists nowhere in code; `probe(at: SettledRun)` accepts a Violated or Unknown run.
- **First seen**: 2026-09-22
- **Reason**: the residue PR #294 left. The `precond-sidecar-run-seam` re-check of 2026-09-21 said it was "recorded on its own entries", but no such entry existed until now. It is a partial deletion test: the walk and the probe rule concentrate in `run.py`, but the per-branch vocabularies (`Assessment`, `CallerOutcome`) and discharge's blame logic stay in each driver. It needs a design pass first, because a `SidecarRunner.assess(...)` returning `Settled | Probed` could end up as wide as the two walks. Harness file names must stay the same, since the tests identify harnesses by name (`tests/precond/test_precond_verify.py:57-58`, `test_discharge.py:92-96`). All pins are esbmc-free. `precondition_ladder` and `sidecar_verify_port` still have no direct test references.

## signature-definition-locator

- **Status**: proposed
- **Score**: 14/25 (leverage 3, locality 4, blast radius 3, heat 1)
- **Files**: ~5–6 estimated
- **Modules**: `src/forseti/properties/signature.py:103-133` (`extract_signature`, regex at `:117-122`); `esbmc/units.py:976-1005` (`find_definition_brace`); callers `properties/harness.py:278`, `core/propose.py:74`, `core/submit.py:81`
- **Summary**: `extract_signature` finds the definition with a raw regex: first match wins, and comments and string literals are not masked. `find_definition_brace` masks them and handles `#if 0` alternatives. Verified by running: `/* int f(long x, long y) { } */ int f(int x) {…}` makes `extract_signature` return `(long x, long y)`, while `find_definition_brace` finds the real definition. The check-path harness then declares the wrong parameters. `_defines_main` (`harness.py:670-689`) already learned this lesson (#95) and uses `find_definition_brace`; the other finder never got the fix.
- **First seen**: 2026-09-22
- **Reason**: corrective, not behaviour-preserving, for sources that hit the case. `tests/properties/test_signature.py:157-172` asserts that `signature.py` imports nothing under `forseti.*`, so the implementer must either relax that pin or move the masking into a stdlib-only leaf both can import. `find_definition_brace` is public (see `scanned-source-carrier`). Not to be confused with `pointer-length-pairing-two-parsers`: this is about *finding* the definition, not pairing parameters.

## semantic-loop-ingestion-verdict

- **Status**: proposed
- **Score**: 15/25 (leverage 2, locality 3, blast radius 3, heat 5)
- **Files**: ~5 estimated
- **Modules**: `src/forseti/core/cli.py:921-927` (a CLI-only override), `core/loop.py:83-112` (`SemanticLoopResult.outcome`), `core/mcp_server.py:295-319`; contract text in `adapters/codex/AGENTS.md:88-94` and `docs/design/0001-harness-portability.md:185-186`
- **Summary**: when every submitted candidate is rejected but the store already holds checkable properties, the loop's `outcome` is `held`. The CLI knows this and exits 1 (comment at `cli.py:922-926`). The MCP `semantic_loop` tool, which Codex's `AGENTS.md` sends the model to, returns `held` with no override. `AGENTS.md` says `empty` means "every candidate was rejected". "Nothing was accepted" gets four different answers across the faces.
- **First seen**: 2026-09-22
- **Reason**: only one copy exists today, so the deletion test is partial: moving the decision into `SemanticLoopResult` (an `accepted_nothing` property) makes it reachable by both faces without a second copy. The behaviour-preserving step changes nothing. The step that actually fixes MCP changes the `--json` and MCP payload, so it is a human decision. Pinned by `tests/core/test_semantic_loop_cli.py:247-305` (esbmc-free). No test covers MCP with every candidate rejected.

## harness-length-bounds-planned-twice

- **Status**: proposed
- **Score**: 14/25 (leverage 2, locality 3, blast radius 1, heat 2)
- **Files**: ~2 estimated
- **Modules**: `src/forseti/properties/harness.py:226`, `:245-267` (`render_property_harness`), `:270-285` (`build_property_harness`), `:390-437` (`plan_length_bounds`); stale docstring `orchestrator/check.py:251-253`
- **Summary**: `build_property_harness` gets its emitted assumes from `render_semantic_harness`, which calls `plan_length_bounds` internally, then calls `plan_length_bounds` again for the reported bounds. "Reported bounds equal emitted assumes" holds only because the function is pure and both calls receive the same arguments. `render_property_harness` is now a one-line wrapper with no caller in `src/`, and `SemanticHarnessWriter`'s docstring still says it adapts to it.
- **First seen**: 2026-09-22
- **Reason**: leverage 2, mostly a tidy-up inside one file. Both functions are exported in `properties.__all__`. Pinned esbmc-free by `tests/properties/test_harness.py:850-1025`.

## max-len-ladder-rule-two-homes

- **Status**: dropped
- **Score**: — (leverage 1)
- **Files**: n/a
- **Modules**: `src/forseti/core/check.py:82-102` (`default_unwind_ladder_above`), `src/forseti/precond/run.py:83-88` (`precondition_ladder`)
- **Summary**: two ladder rules both derive `max_len + 1`, linked only by the comment at `check.py:97`.
- **First seen**: 2026-09-22
- **Reason**: Leverage 1. The shapes differ on purpose: `check` has a user-chosen base `unwind` shared with scalar-only properties, and `synth` does not. A shared helper would take that difference as a parameter, and merging them changes one path's ladder. The *behavioural* version is a human's call: trim or extend the ladder per property from `RenderedHarness.length_bounds`, so a capped property stops spending rungs ≤ `max_len` that cannot settle and `--max-len 32` stops extending a scalar-only unit's ladder. It changes the settled `k` on the `--json` wire and in `property.verdict`.
